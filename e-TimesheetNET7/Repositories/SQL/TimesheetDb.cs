...
namespace e_TimesheetNET7.Repositories.SQL
{
    ...
    public class TimesheetDb : ITimesheetDb
    {
        ...
        public async Task<bool> InsertTimesheet(TimesheetData tsData)
        {
            ...
            # region Cek Tgl Mulai dan Tahun di TS Detail bds.NIP TS Header
            ...
            #region cek InternalTSNo pada TS Header
            var cekHeader = "SELECT COUNT(*) FROM SAP_TrTimeSheetHeader where InternalTSNo=?";
            var tsNo = new DynamicParameters();
            tsNo.Add("InternalTSNo", tsData.Header.InternalTSNo);
            var isHeaderExist = await conn.QueryFirstOrDefaultAsync<int>(cekHeader, tsNo);
            #endregion
